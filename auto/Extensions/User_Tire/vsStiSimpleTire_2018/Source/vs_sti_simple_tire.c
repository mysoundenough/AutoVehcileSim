...
char *make_full_path(char *inp_str)
{
  static char path_file[FILENAME_MAX];
  if(strstr(inp_str, ":\\")           ||  // Windows with dirver letter, absolute path
     strncmp(inp_str, "\\\\", 2) == 0 ||  // Network path, absolute path
     strncmp(inp_str, ".\\", 2)  == 0 ||  // Windows from current working path     
     strncmp(inp_str, "./", 2)   == 0 ||  // Linux from current working path     
     inp_str[0] == '/')                   // Linux absolute path
    {
     strcpy(path_file, inp_str);
    }
  else
    {
     char *p = gDatabaseDir();
     ...
     if(strpbrk(&p[i-2], "\\"))  // There is a backslash "\" at the end
       sprintf(path_file, "%s%s", p, inp_str);
     else
       sprintf(path_file, "%s\\%s", p, inp_str);
  #else  // Linux or QNX
     if(strpbrk(&p[i-2], "/"))
...
int check_file_exist(char *file_name, char *file_type)
{
  int ret_val = 1;
#ifndef _VS_RT
  FILE *ifile = fopen(file_name, "r");
    if (!ifile)
    {
      ...
      if(strcmp(file_type, "property") == 0)
        sprintf(buf, "a property file for User STI tire model was not found.\nFile full path: %s\n", file_name);
      else
        sprintf(buf, "a road data file for User STI tire model was not found.\nFile full path: %s\n", file_name);
...
vs_bool vs_sti_scan(char *key, char *buffer) {
  ...
  if (!strcmp(key, "USER_STI_DATA")) // full path name
  {
    strcpy (sUserTire[itire]->chtdst, make_full_path(buffer));
    gPrintEcho (sUserLogFile,"USER_STI_DATA %s\n", sUserTire[itire]->chtdst);
    if(check_file_exist(sUserTire[itire]->chtdst, "property") == 0) return TRUE;
    // Check if TRD file
    if (strstr(sUserTire[itire]->chtdst, ".TRD") ||
        strstr(sUserTire[itire]->chtdst, ".trd"))
    {
  ...
  else if (!strcmp(key, "USER_STI_ROAD"))  // full path name
  {
    strcpy (sUserTire[itire]->chtdst, make_full_path(buffer));
    check_file_exist(sUserTire[itire]->chtdst, "road data");
    return TRUE;
  }
...
void Install (int nTires, int duals) {
  int i, job = 1;
#ifndef _VS_RT
  char log_path[FILENAME_MAX], strMap[20];
#endif
  sUserTireN = nTires;
